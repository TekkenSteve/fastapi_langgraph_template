...
from agent_server.config.graph_config import load_checkpointer_config
from agent_server.config.settings import settings
from agent_server.domain.threads import MAX_TTL_MINUTES
...
class ThreadTTLConfig(BaseModel):
    """Validated TTL configuration merged from env and langgraph.json."""
...
def get_thread_ttl_config() -> ThreadTTLConfig | None:
    """Resolve TTL config: THREAD_TTL env var wins over langgraph.json.
    ...
    """
    # LANGGRAPH_THREAD_TTL is a migration alias; THREAD_TTL wins when both are set.
    raw = settings.thread_ttl.THREAD_TTL

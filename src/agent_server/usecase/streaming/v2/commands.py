...
from agent_server.usecase.streaming.v2.protocol import ErrorCode, build_error, build_success
...
# Protocol keys plus server extensions per handler. Unknown keys are logged, never
# silently dropped; tests/unit/test_usecase/test_event_streaming/test_spec_params.py
# pins these against the vendored Agent Protocol param shapes.
...
def _warn_unknown_keys(method: str, params: dict[str, Any], known: frozenset[str]) -> None:
    """Log params outside the declared key set so a new SDK field is never silent."""
...
def _is_goto_target(target: Any) -> bool:
    """A goto target is a non-empty node name or a ``{node, input}`` object."""

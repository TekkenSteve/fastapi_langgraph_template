...
registration from :mod:`agent_server.auth.registry`, so a route opts *out*
...
from agent_server.auth.registry import is_self_dispatching, lookup_route_auth
...
_dispatched: ContextVar[frozenset[tuple[str, str]]] = ContextVar("agent_server_auth_dispatched", default=frozenset())
...
def _wire_route(route: APIRoute) -> int:
    ...
    if getattr(route, "_agent_server_auth_wired", False):
        return 0

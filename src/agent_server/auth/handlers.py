...
from agent_server.auth.middleware import get_auth_instance
from agent_server.domain.user import User


class AuthContextWrapper:
    """Wrapper to convert server User model to AuthContext.
    ...
    def __init__(
        ...
        Args:
            user: Authenticated user from the server's User model
...
async def handle_event(
    ...
    **Default Behavior (Non-Interruptive):**
    ...
    This ensures that developers using the server without custom authorization
    ...
    if auth is None:
        ...
        # This ensures the server works out-of-the-box without interruption
        return None
    ...
    if handler is None:
        ...
        # Developers can run without defining handlers - it won't break
        return None

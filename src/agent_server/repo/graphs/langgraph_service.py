...
from agent_server.repo.graphs.graph_factory import (
    AccessContext,
...
def _module_name_for(graph_id: str) -> str:
    ...
    placing the result under a private ``user_graphs.`` namespace.
    """
    safe_id = graph_id.replace(".", "_").replace("/", "_").replace("-", "_")
    return f"user_graphs.{safe_id}"
...
class LangGraphService:
    ...
    def __init__(self, config_path: str | None = None) -> None:
        self.config_path = Path(config_path) if config_path else Path("langgraph.json")
    ...
    async def initialize(self) -> None:
        ...
        2) Shared resolution (GRAPHS_CONFIG env var → langgraph.json)
        """
        from agent_server.config.graph_config import _resolve_config_path
        ...
        if not resolved_path or not resolved_path.exists():
            raise ValueError("Configuration file not found. Expected one of: GRAPHS_CONFIG path or ./langgraph.json")
    ...
    def _load_graph_registry(self) -> None:
        ...
        # "a_b" both map to user_graphs.a_b in sys.modules).
    ...
    async def _ensure_default_assistants(self) -> None:
        ...
        from agent_server.repo.orm import AssistantVersion as AssistantVersionORM
    ...
    async def _get_base_graph(self, graph_id: str) -> Pregel:
        ...
        @param graph_id: The graph identifier from langgraph.json
    ...
    async def get_graph(
        ...
        *,
        ...
        Args:
            graph_id: The graph identifier from langgraph.json.

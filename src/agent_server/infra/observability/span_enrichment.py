...
# time, so we filter at this layer and emit a server-level warning
...
_trace_attrs: contextvars.ContextVar[dict[str, str | int | float | bool] | None] = contextvars.ContextVar(
    "agent_server_otel_trace_attrs", default=None
)
...
class SpanEnrichmentProcessor(SpanProcessor):
    ...
    Reads from the ``agent_server_otel_trace_attrs`` context variable and sets

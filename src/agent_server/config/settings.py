...
from agent_server.config.constants import MULTIHOST_URL_RE
...
class AppSettings(EnvBase):
    ...
    # App logic
    GRAPHS_CONFIG: str = "langgraph.json"  # Default config file path
...
class ObservabilitySettings(EnvBase):
    ...
    # General OTEL Config
    OTEL_SERVICE_NAME: str = "agent-server"
...
class RedisSettings(EnvBase):
    ...
    REDIS_CHANNEL_PREFIX: str = "agent_server:run:"
...
class WorkerSettings(EnvBase):
    ...
    WORKER_QUEUE_KEY: str = "agent_server:jobs"
...
class ThreadTTLSettings(EnvBase):
    ...
    THREAD_TTL is either a bare number (default_ttl in minutes) or a
    JSON object with any of: strategy, default_ttl, sweep_interval_minutes,
    sweep_limit. When set it replaces the langgraph.json checkpointer.ttl block
    entirely. LANGGRAPH_THREAD_TTL is accepted as a fallback alias so env
    files migrated from LangGraph Platform work unchanged; THREAD_TTL
    wins when both are set. Parsed and validated in usecase.thread_ttl.
    """

    THREAD_TTL: str | None = None

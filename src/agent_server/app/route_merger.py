...
def merge_lifespans(user_app: FastAPI, core_lifespan: Callable) -> FastAPI:
    """Merge user lifespan with the server's core lifespan.

    Both lifespans will run, with core lifespan wrapping user lifespan.
    This ensures the server's initialization (database, services) happens before
    user initialization, and cleanup happens in reverse order.

    Args:
        user_app: User's FastAPI/Starlette application
        core_lifespan: the server's core lifespan context manager
...
def merge_exception_handlers(user_app: FastAPI, core_exception_handlers: Mapping[type[Exception], Callable]) -> FastAPI:
    ...
    Args:
        user_app: User's FastAPI/Starlette application
        core_exception_handlers: the server's core exception handlers

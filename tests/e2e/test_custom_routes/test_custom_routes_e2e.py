...
These tests require a running server with custom routes configured.
To run these tests:
1. Ensure langgraph.json has http.app pointing to custom_routes_example.py
...
def test_core_routes_still_work():
    """Test that core server routes still work alongside custom routes"""

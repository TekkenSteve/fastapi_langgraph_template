...
Run:
  cd fastapi_langgraph_template
  docker compose -f deployments/test/docker-compose.multi.yml up -d --build
  uv run python tests/e2e/multi_instance/test_stress.py
"""
...
_REPO_ROOT = Path(__file__).resolve().parents[3]  # tests/e2e/multi_instance -> repo root
...
async def stress_single_worker_backpressure() -> StressResult:
    ...
    log("  Killing app-b to reduce to 2 workers...")
    subprocess.run(
        ["docker", "compose", "-f", "docker-compose.multi.yml", "kill", "app-b"],
        capture_output=True,
        check=True,
        cwd=COMPOSE_DIR,
    )
    ...
    try:
        ...
        # Restart instance B
        log("  Restarting app-b...")
        subprocess.run(
            ["docker", "compose", "-f", "docker-compose.multi.yml", "up", "-d", "app-b"],
            capture_output=True,
            check=True,
            cwd=COMPOSE_DIR,
        )

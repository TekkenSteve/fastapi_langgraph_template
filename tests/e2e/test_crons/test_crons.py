...
async def test_cron_search_and_count_filter_by_metadata() -> None:
    """Cron search/count honor the metadata containment filter."""
    client = get_e2e_client()
    assistant = await client.assistants.create(
        graph_id="agent",
    ...
    try:
            ...
            created = await _create_cron_via_http(
                {
                    ...
                    "enabled": False,
                    "metadata": {"team": team},
                    "input": {"messages": [{"role": "user", "content": f"cron-meta-{team}-{uuid4()}"}]},
                }
            )
        ...
        assert await client.crons.count(assistant_id=assistant_id, metadata={"team": "research"}) == 1
    finally:

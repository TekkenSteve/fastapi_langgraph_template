...
These tests hit a real running server and exercise the ``POST /runs/wait``,
``POST /runs/stream``, and ``POST /runs`` endpoints that accept no thread_id.

Requirements:
  - A running server (``make dev`` or ``docker compose up``)
  - The ``agent`` graph deployed in the server's ``langgraph.json``
"""

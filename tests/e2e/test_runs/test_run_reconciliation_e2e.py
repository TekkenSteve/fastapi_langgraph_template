...
from agent_server.usecase.execution.run_status import finalize_run
...
async def test_stale_worker_cannot_overwrite_reconciled_interruption() -> None:
    ...
    async with _seed_run(
        ...
        maker = async_sessionmaker(engine, expire_on_commit=False)
        try:
            with patch("agent_server.usecase.execution.run_status._get_session_maker", return_value=maker):
    ...
    async with _seed_run(run_status="interrupted", thread_status="idle") as (thread_id, run_id):
        ...
        try:
            with (
                ...
                patch("agent_server.usecase.execution.worker_executor.settings") as mock_settings,
                patch("agent_server.usecase.execution.worker_executor._get_session_maker", return_value=maker),
                patch("agent_server.usecase.execution.run_status._get_session_maker", return_value=maker),
            ):

...
from agent_server.infra.observability import metrics as metrics_module
...
def test_instrumented_request_on_included_router_route_does_not_500(
    ...
    a match, which raised AttributeError on every request with the old pin. The server
...
def test_reaper_counter_exposed_in_default_registry() -> None:
    ...
    assert "agent_server_reaper_recovered_runs_total" in body

...
from agent_server.domain.crons import CronResponse
from agent_server.usecase.cron.service import get_cron_service
...
def client(mock_cron_service: AsyncMock) -> Iterator[TestClient]:
    ...
    from agent_server.controller.http.routers import crons as crons_module
    ...
    with (
        patch("agent_server.controller.http.routers.crons.handle_event", new_callable=AsyncMock) as mock_handle,
        patch("agent_server.controller.http.routers.crons._trigger_first_run", new_callable=AsyncMock) as mock_trigger,
    ):
...
class TestCreateCronForThreadOwnership:
    ...
    def test_returns_404_when_thread_owned_by_other_user(self, mock_cron_service: AsyncMock) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.crons.handle_event", new_callable=AsyncMock),
            patch("agent_server.controller.http.routers.crons._trigger_first_run", new_callable=AsyncMock),
        ):

...
from agent_server.repo.graphs.langgraph_service import LangGraphService
...
class TestLangGraphServiceRealFiles:
    ...
    async def test_initialize_with_real_config_file(self):
        ...
        with TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "langgraph.json"
    ...
    async def test_initialize_env_var_with_real_file(self, monkeypatch):
        """Test initialization with GRAPHS_CONFIG pointing to real file"""
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(settings.app, "GRAPHS_CONFIG", str(config_path)),
                patch("agent_server.repo.graphs.langgraph_service.LangGraphService._ensure_default_assistants"),
                patch("agent_server.repo.graphs.langgraph_service.LangGraphService._load_all_graph_modules"),
            ):
    ...
    async def test_initialize_fallback_to_langgraph_json(self, monkeypatch):
        """Test config discovery falls back to ./langgraph.json in the CWD"""
        import sys
        ...
        try:
            ...
            # Ensure GRAPHS_CONFIG is None so fallback logic triggers
            with (
...
class TestLangGraphServiceDatabase:
    ...
    async def test_get_graph_database_integration(self):
        ...
        with (
            ...
            patch("agent_server.repo.database.db_manager") as mock_db_manager,
        ):

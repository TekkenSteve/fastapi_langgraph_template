...
from agent_server.auth import handlers as auth_handlers
from agent_server.auth.deps import get_current_user, require_auth
from agent_server.auth.enforcement import (
...
def _build_app(auth: Auth, monkeypatch: pytest.MonkeyPatch) -> FastAPI:
    ...
    # `agent_server.auth`, which breaks string-target resolution in a full run.
    monkeypatch.setattr(auth_handlers, "get_auth_instance", lambda: auth)

    from agent_server.controller.http.routers import (
        assistants,
        crons,
        event_streaming,
        runs,
...
def test_no_auth_configured_still_allows_requests(monkeypatch: pytest.MonkeyPatch) -> None:
    """A server without an auth.py must keep working — dispatch is a no-op there."""
    monkeypatch.setattr(auth_handlers, "get_auth_instance", lambda: None)

    from agent_server.controller.http.routers import assistants

...
from agent_server.usecase.cron.scheduler import CronScheduler
...
class TestSchedulerTick:
    ...
    async def test_find_due_crons_delegates_to_cron_service(self) -> None:
        ...
        with patch("agent_server.usecase.cron.scheduler.CronService") as mock_service_cls:
    ...
    async def test_tick_no_due_crons(self) -> None:
        ...
        with (
            patch(
                "agent_server.usecase.cron.scheduler._get_session_maker",
...
class TestTickTimezone:
    ...
    async def test_fire_cron_passes_timezone_from_payload(self) -> None:
        ...
        with (
            patch(
                "agent_server.usecase.cron.scheduler._prepare_run",
                new_callable=AsyncMock,
                return_value=("run-1", Mock(), None),
            ),
            patch(
                "agent_server.usecase.cron.service._compute_next_run",
...
class TestFireCron:
    ...
    async def test_schedules_cleanup_for_stateless_cron_by_default(self) -> None:
        ...
        with (
            patch("agent_server.usecase.cron.scheduler.uuid4", return_value="eph-thread-1"),
            patch(
                "agent_server.usecase.cron.scheduler._prepare_run",
                new_callable=AsyncMock,
                return_value=("run-1", Mock(), None),
            ),
            patch("agent_server.usecase.cron.scheduler.schedule_background_cleanup") as mock_schedule,
        ):
            await scheduler._fire_cron(mock_session, cron)
    ...
    async def test_skips_cleanup_when_on_run_completed_is_keep(self) -> None:
        ...
        with (
            patch("agent_server.usecase.cron.scheduler.uuid4", return_value="eph-thread-keep"),
            patch(
                "agent_server.usecase.cron.scheduler._prepare_run",
                new_callable=AsyncMock,
                return_value=("run-1", Mock(), None),
    ...
    async def test_deletes_stateless_thread_when_run_setup_fails(self) -> None:
        ...
        with (
            patch("agent_server.usecase.cron.scheduler.uuid4", return_value="eph-thread-fail"),
            patch(
                "agent_server.usecase.cron.scheduler._prepare_run",
                new_callable=AsyncMock,
            ...
            patch(
                "agent_server.usecase.cron.scheduler.delete_thread_by_id",
                new_callable=AsyncMock,
            ) as mock_delete,
        ):
            await scheduler._fire_cron(mock_session, cron)
    ...
    async def test_handles_http_exception_in_run_creation(self) -> None:
        ...
        with (
            ...
            patch(
                "agent_server.usecase.cron.scheduler.CronScheduler._cleanup_failed_stateless_thread",
                new_callable=AsyncMock,
            ),
        ):
...
class TestSchedulerLoop:
    ...
    async def test_loop_stops_when_running_is_false(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.cron.scheduler.settings") as mock_settings,
        ):
    ...
    async def test_loop_handles_cancelled_error_during_sleep(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.cron.scheduler.settings") as mock_settings,
            patch("agent_server.usecase.cron.scheduler.asyncio.sleep", side_effect=cancelling_sleep),
        ):
            mock_settings.cron.CRON_POLL_INTERVAL_SECONDS = 0.01

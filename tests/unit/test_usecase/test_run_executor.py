...
from agent_server.usecase.execution import run_executor as run_executor_module
from agent_server.usecase.execution.run_executor import (
    _GraphResult,
...
from agent_server.usecase.execution.run_job import RunExecution, RunIdentity, RunJob
...
class TestExecuteRunSuccess:
    ...
    async def test_success_path_updates_status_and_signals(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_executor.get_langgraph_service", return_value=mock_service),
            patch("agent_server.usecase.execution.run_executor.start_run", mock_start),
            patch("agent_server.usecase.execution.run_executor.finalize_run", mock_finalize),
            patch("agent_server.usecase.execution.run_executor.streaming_service") as mock_streaming,
            patch("agent_server.usecase.execution.run_executor.stream_graph_events", return_value=_empty_async_gen()),
            patch(
                "agent_server.usecase.execution.run_executor._signal_end_event", new_callable=AsyncMock
            ) as mock_signal_end,
            patch("agent_server.usecase.execution.run_executor._signal_run_done", new_callable=AsyncMock),
            patch("agent_server.usecase.execution.run_executor.with_auth_ctx") as mock_auth,
        ):
...
class TestExecuteRunCancelledError:
    ...
    async def test_cancelled_error_sets_interrupted_and_signals(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.execution.run_executor._signal_run_done", new_callable=AsyncMock),
            patch(
                "agent_server.usecase.execution.run_executor._stream_graph",
    ...
    async def test_timeout_cancel_sets_error_and_signals(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_executor.start_run", new_callable=AsyncMock, return_value=True),
            patch("agent_server.usecase.execution.run_executor.finalize_run", mock_finalize),
            patch("agent_server.usecase.execution.run_executor.streaming_service") as mock_streaming,
            patch("agent_server.usecase.execution.run_executor._signal_run_done", new_callable=AsyncMock),
            patch(
...
class TestSignalEndEvent:
    ...
    async def test_publishes_end_event(self) -> None:
        ...
        with patch("agent_server.usecase.execution.run_executor.broker_manager") as mock_bm:
...
class TestSignalRunDone:
    ...
    async def test_sets_redis_key(self) -> None:
        ...
        with patch("agent_server.usecase.execution.run_executor.redis_manager") as mock_rm:
    ...
    async def test_uses_configured_channel_prefix(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_executor.redis_manager") as mock_rm,
            patch("agent_server.usecase.execution.run_executor.settings") as mock_settings,
        ):
            mock_rm.get_client.return_value = mock_client
            mock_settings.redis.REDIS_CHANNEL_PREFIX = "agent_server:agent-foo:run:"

            await _signal_run_done("run-1")

        key = mock_client.set.await_args.args[0]
        assert key == "agent_server:agent-foo:run:done:run-1"
...
class TestLeaseLossCancellation:
    ...
    async def test_lease_loss_cancel_skips_finalize_and_signal(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.execution.run_executor.finalize_run", mock_finalize),
            patch("agent_server.usecase.execution.run_executor.streaming_service") as mock_streaming,
            patch("agent_server.usecase.execution.run_executor._signal_run_done", mock_signal_done),
            patch(
                "agent_server.usecase.execution.run_executor._stream_graph",
                new_callable=AsyncMock,
                side_effect=asyncio.CancelledError,
...
class TestShutdownDrainCancellation:
    ...
    async def test_shutdown_flag_is_cleared_after_cancel(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_executor.start_run", new_callable=AsyncMock, return_value=True),
            patch("agent_server.usecase.execution.run_executor.finalize_run", new_callable=AsyncMock),
            patch("agent_server.usecase.execution.run_executor.streaming_service") as mock_streaming,
            patch("agent_server.usecase.execution.run_executor._signal_run_done", new_callable=AsyncMock),
            patch(
                "agent_server.usecase.execution.run_executor._stream_graph",
...
class TestTerminalStateRaces:
    ...
    async def test_terminal_run_does_not_start_graph_execution(self) -> None:
        with (
            patch("agent_server.usecase.execution.run_executor.start_run", new_callable=AsyncMock, return_value=False),
            patch("agent_server.usecase.execution.run_executor._stream_graph", new_callable=AsyncMock) as mock_stream,
            patch("agent_server.usecase.execution.run_executor.finalize_run", new_callable=AsyncMock) as mock_finalize,
            patch("agent_server.usecase.execution.run_executor.streaming_service") as mock_streaming,
            patch("agent_server.usecase.execution.run_executor._signal_run_done", new_callable=AsyncMock),
        ):
            mock_streaming.cleanup_run = AsyncMock()
    ...
    async def test_lost_finalization_race_does_not_signal_success(self) -> None:
        ...
        with (
            ...
            patch(
                "agent_server.usecase.execution.run_executor.finalize_run", new_callable=AsyncMock, return_value=False
            ),
            patch(
                "agent_server.usecase.execution.run_executor._signal_end_event", new_callable=AsyncMock
            ) as mock_signal_end,

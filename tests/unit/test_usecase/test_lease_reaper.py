...
from agent_server.infra.observability.metrics import REAPER_RECOVERED_RUNS
from agent_server.usecase.execution.lease_reaper import LeaseReaper
...
class TestFindRecoverable:
    ...
    async def test_returns_crashed_and_stuck_separately(self) -> None:
        ...
        with patch("agent_server.usecase.execution.lease_reaper._get_session_maker", return_value=maker):
...
class TestRecoverCrashedRuns:
    ...
    async def test_classifies_and_transitions_under_one_transaction(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.lease_reaper._get_session_maker", return_value=maker),
            patch("agent_server.usecase.execution.lease_reaper.settings") as mock_settings,
            patch(
                "agent_server.usecase.execution.lease_reaper.set_thread_status_if_no_active_runs",
...
class TestReenqueue:
    ...
    async def test_pushes_to_redis(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.lease_reaper.redis_manager") as mock_rm,
            patch("agent_server.usecase.execution.lease_reaper.settings") as mock_settings,
        ):
...
class TestStartStop:
    ...
    async def test_start_creates_background_task(self) -> None:
        ...
        with patch("agent_server.usecase.execution.lease_reaper.settings") as mock_settings:

...
from agent_server.controller.http.routers.threads import (
    get_thread_state_at_checkpoint,
    get_thread_state_at_checkpoint_post,
)
from agent_server.domain import ThreadCheckpoint, ThreadCheckpointPostRequest, User
...
class TestGetThreadStateAtCheckpoint:
    ...
    async def test_success_with_subgraphs_passed_to_service(self):
        ...
        with (
            patch("agent_server.repo.graphs.langgraph_service.get_langgraph_service") as mock_service,
            patch(
                "agent_server.repo.graphs.langgraph_service.create_thread_config",
                return_value=config,
            ),
            patch(
                "agent_server.controller.http.routers.threads.thread_state_service.convert_snapshot_to_thread_state",
...
class TestGetThreadStateAtCheckpointPost:
    ...
    async def test_success_with_checkpoint_ns(self):
        ...
        with patch(
            "agent_server.controller.http.routers.threads.get_thread_state_at_checkpoint",
            return_value=mock_thread_state,
        ) as mock_get:

...
from agent_server.usecase.execution.run_executor import _shutdown_cancellations, _timeout_cancellations
from agent_server.usecase.execution.run_job import RunBehavior, RunExecution, RunIdentity, RunJob
from agent_server.usecase.execution.worker_executor import (
    WorkerExecutor,
    _acquire_and_load,
    _heartbeat_loop,
    _is_run_terminal,
...
MODULE = "agent_server.usecase.execution.worker_executor"
...
class TestWorkerExecutorSubmit:
    ...
    async def test_pushes_run_id_to_redis(self) -> None:
        ...
        with (
            ...
            mock_settings.worker.WORKER_QUEUE_KEY = "agent_server:jobs"

            executor = WorkerExecutor()
            await executor.submit(job)

        mock_client.rpush.assert_awaited_once_with("agent_server:jobs", "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee")
...
class TestWorkerExecutorWaitForCompletion:
    ...
    async def test_done_key_uses_configured_channel_prefix(self) -> None:
        ...
        mock_client.exists.assert_awaited_once_with("agent_server:agent-foo:run:done:run-1")
...
class TestDequeue:
    ...
    async def test_returns_run_id_on_queue_hit(self) -> None:
        blpop = AsyncMock(return_value=("agent_server:worker:queue", "run-123"))
...
class TestRequeueDrainedRuns:
    ...
    async def test_resets_rows_and_pushes_to_queue(self) -> None:
        ...
        assert pushed == [("agent_server:jobs", "run-1"), ("agent_server:jobs", "run-2")]

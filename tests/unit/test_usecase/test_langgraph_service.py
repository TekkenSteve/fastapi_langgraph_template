...
from agent_server.config import graph_config as config_module
from agent_server.config.settings import settings
from agent_server.repo.graphs.langgraph_service import (
    LangGraphService,
    create_run_config,
...
class TestLangGraphServiceInit:
    ...
    def test_init_default_config_path(self):
        ...
        # Ensure GRAPHS_CONFIG is None so it defaults to langgraph.json
        with patch.object(settings.app, "GRAPHS_CONFIG", None):
            service = LangGraphService()
            assert service.config_path == Path("langgraph.json")
...
class TestLangGraphServiceConfig:
    ...
    async def test_initialize_env_var_override(self):
        """Test config loading with GRAPHS_CONFIG env var"""
        config_data = {"graphs": {"test": "./graphs/test.py:graph"}}
        env_path = "/env/path/config.json"

        # A sibling test reloads agent_server.config.settings, so patch the settings
        # object agent_server.config.graph_config actually holds (#512).
        with (
            patch.object(config_module.settings.app, "GRAPHS_CONFIG", env_path),
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.open", mock_open(read_data=json.dumps(config_data))),
            patch("agent_server.repo.graphs.langgraph_service.LangGraphService._ensure_default_assistants"),
            patch("agent_server.repo.graphs.langgraph_service.LangGraphService._load_all_graph_modules"),
        ):
            service = LangGraphService()
    ...
    async def test_initialize_langgraph_json_fallback(self):
        """Test config loading falls back to ./langgraph.json in the CWD"""
        config_data = {"graphs": {"test": "./graphs/test.py:graph"}}

        # Ensure GRAPHS_CONFIG is None so it falls back to default discovery
        with (
            patch.object(settings.app, "GRAPHS_CONFIG", None),
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.open", mock_open(read_data=json.dumps(config_data))),
            patch("agent_server.repo.graphs.langgraph_service.LangGraphService._ensure_default_assistants"),
            patch("agent_server.repo.graphs.langgraph_service.LangGraphService._load_all_graph_modules"),
...
class TestLangGraphServiceGraphs:
    ...
    async def test_get_graph_for_validation_success(self):
        ...
        import agent_server.repo.graphs.langgraph_service as lgs_module
    ...
    async def test_get_graph_context_manager(self):
        ...
        with patch("agent_server.repo.database.db_manager") as mock_db_manager:
...
class TestLangGraphServiceConfigs:
    ...
    def test_create_run_config(self):
        ...
        with patch(
            "agent_server.repo.graphs.langgraph_service.get_tracing_callbacks",
            return_value=[],
        ):
    ...
    def test_create_run_config_with_tracing_callbacks(self):
        ...
        with (
            ...
            patch(
                "agent_server.repo.graphs.langgraph_service.get_tracing_metadata",
                return_value={
        ...
        assert "agent_server_run" in result["metadata"]["langfuse_tags"]
...
async def test_get_graph_context_manager_injects_checkpointer(monkeypatch):
    ...
    import agent_server.repo.database as dbmod
...
class TestSetupDependencies:
    ...
    def test_setup_dependencies_adds_paths(self, tmp_path):
        ...
        service.config_path = tmp_path / "langgraph.json"
    ...
    def test_setup_dependencies_relative_path(self, tmp_path):
        ...
        service = LangGraphService()
        service.config = config
        service.config_path = config_dir / "langgraph.json"
    ...
    def test_setup_dependencies_empty_config(self):
        ...
        service = LangGraphService()
        service.config = config
        service.config_path = Path("langgraph.json")

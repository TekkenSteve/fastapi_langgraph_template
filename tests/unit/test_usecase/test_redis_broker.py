...
from agent_server.usecase.streaming.redis_broker import (
    _PUT_MAX_ATTEMPTS,
    RedisBrokerManager,
    RedisRunBroker,
...
class TestRedisRunBroker:
    ...
    def _make_broker(self, run_id: str = "run-123") -> RedisRunBroker:
        return RedisRunBroker(
            run_id,
            f"agent_server:run:{run_id}",
            f"agent_server:run:cache:{run_id}",
            f"agent_server:run:counter:{run_id}",
        )
    ...
    async def test_put_publishes_and_caches(self) -> None:
        ...
        with patch("agent_server.usecase.streaming.redis_broker.redis_manager") as mock_rm:
            ...
            assert channel == "agent_server:run:run-123"
            ...
            assert cache_key == "agent_server:run:cache:run-123"
            ...
            mock_pipe.incr.assert_called_once_with("agent_server:run:counter:run-123")
    ...
    async def test_put_handles_redis_failure_gracefully(self) -> None:
        ...
        with (
            patch("agent_server.usecase.streaming.redis_broker.redis_manager") as mock_rm,
            patch("agent_server.usecase.streaming.redis_broker.asyncio.sleep", new_callable=AsyncMock),
        ):
            mock_rm.get_client.return_value = mock_client
    ...
    async def test_put_retries_cache_write_then_succeeds(self) -> None:
        ...
        with (
            patch("agent_server.usecase.streaming.redis_broker.redis_manager") as mock_rm,
            patch("agent_server.usecase.streaming.redis_broker.asyncio.sleep", new_callable=AsyncMock) as mock_sleep,
        ):
            mock_rm.get_client.return_value = mock_client
...
class TestRedisBrokerManager:
    ...
    def test_broker_has_correct_keys(self) -> None:
        ...
        assert broker._cache_key == "agent_server:run:cache:run-123"
        assert broker._channel == "agent_server:run:run-123"
        assert broker._counter_key == "agent_server:run:counter:run-123"
    ...
    async def test_get_event_sequence_reads_from_redis(self) -> None:
        ...
        assert result == 42
        mock_client.get.assert_awaited_once_with("agent_server:run:counter:run-123")
    ...
    async def test_request_cancel_publishes_to_redis(self) -> None:
        ...
        with patch("agent_server.usecase.streaming.redis_broker.redis_manager") as mock_rm:
            ...
            channel, message = mock_client.publish.call_args[0]
            assert channel == "agent_server:run:cancel"
    ...
    async def test_execute_cancel_cancels_local_task(self) -> None:
        ...
        with (
            patch.dict("agent_server.usecase.execution.active_runs.active_runs", {"run-123": mock_task}, clear=True),
            patch("agent_server.usecase.streaming.redis_broker.explicit_run_cancellations", set()) as cancellations,
    ...
    async def test_execute_cancel_ignores_missing_task(self) -> None:
        ...
        with patch.dict("agent_server.usecase.execution.active_runs.active_runs", {}, clear=True):
            await manager._execute_cancel("run-123")
            # Should not raise

...
from agent_server.infra.observability.metrics import THREAD_TTL_SWEPT
from agent_server.usecase.thread_ttl import (
    ThreadTTLConfig,
    ThreadTTLSweeper,
    _apply_strategy,
    _expired_claim_stmt,
...
def _no_env_ttl(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", None)
...
class TestResolveConfig:
    ...
    def test_loads_from_agent_server_json_checkpointer_ttl(
    ...
    def test_env_bare_number_sets_default_ttl(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "43200")
    ...
    def test_env_replaces_agent_server_json_block_entirely(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "120")
    ...
    def test_invalid_env_json_raises(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "{not json")
    ...
    def test_invalid_strategy_raises(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", json.dumps({"strategy": "purge"}))
    ...
    def test_non_positive_default_ttl_raises(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "0")
    ...
    def test_agent_server_var_wins_over_langgraph_alias(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "111")
        monkeypatch.setattr(settings.thread_ttl, "LANGGRAPH_THREAD_TTL", "777")
    ...
    def test_blank_env_falls_back_to_json(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(settings.thread_ttl, "THREAD_TTL", "   ")
...
class TestDeleteStrategy:
    ...
    async def test_deletes_checkpoints_before_thread_row(self) -> None:
        ...
        with patch("agent_server.usecase.thread_ttl.db_manager", db):
...
class TestKeepLatest:
    ...
    async def test_raises_when_db_not_initialized(self) -> None:
        ...
        with (
            patch("agent_server.usecase.thread_ttl.db_manager", db),
...
class TestSweepLimit:
    ...
    async def test_stops_at_sweep_limit(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("agent_server.usecase.thread_ttl.get_thread_ttl_config", lambda: config)
        ...
        with (
            patch("agent_server.usecase.thread_ttl._get_session_maker", return_value=maker),
            patch(
                "agent_server.usecase.thread_ttl._process_expired_batch",
    ...
    async def test_tick_is_noop_without_config(self, monkeypatch: pytest.MonkeyPatch) -> None:
        sweeper = ThreadTTLSweeper()
        monkeypatch.setattr("agent_server.usecase.thread_ttl.get_thread_ttl_config", lambda: None)

        with patch("agent_server.usecase.thread_ttl._process_expired_batch", new_callable=AsyncMock) as mock_batch:
            await sweeper._tick()

...
class TestReplayRetention:
    ...
    @pytest.mark.asyncio
    async def test_mark_finished_keeps_the_first_completion_time(self) -> None:
        """Repeated cleanup calls must not restart the replay window."""
        broker = RunBroker("run-1")
        broker.mark_finished()
        first = broker._finished_at

        broker.mark_finished()

        assert broker._finished_at == first

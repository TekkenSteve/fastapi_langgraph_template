...
def test_merge_jsonb_and_should_skip_event():
    ...
    from agent_server.usecase.execution.run_utils import _merge_jsonb, _should_skip_event
...
async def test_filter_no_schema_returns_same():
    from agent_server.usecase.execution import run_utils
...
class TestMapCommandToLangGraph:
    """A goto target without ``input`` must not KeyError on the Send mapping."""

    def test_goto_send_without_input_maps_to_none_input(self):
    ...
    def test_goto_send_with_input_keeps_input(self):
        from langgraph.types import Send

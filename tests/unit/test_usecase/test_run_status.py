...
from agent_server.usecase.execution.run_status import (
    _safe_serialize,
    finalize_run,
...
class TestStartRun:
    ...
    async def test_starts_active_run(self) -> None:
        ...
        with patch(
            "agent_server.usecase.execution.run_status._get_session_maker",
            return_value=_make_mock_session_maker(session),
        ):
...
class TestFinalizeRun:
    ...
    async def test_finalizes_only_an_active_run(self) -> None:
        ...
        with (
            ...
            patch(
                "agent_server.usecase.execution.run_status.set_thread_status_if_no_active_runs",
    ...
    async def test_serializes_output_and_records_error_when_provided(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.execution.run_status._safe_serialize", return_value={"key": "val"}) as mock_ser,
            patch(
                "agent_server.usecase.execution.run_status.set_thread_status_if_no_active_runs",
                new_callable=AsyncMock,
            ),
    ...
    async def test_omits_output_serialization_when_not_provided(self) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.execution.run_status._safe_serialize") as mock_ser,
            patch(
                "agent_server.usecase.execution.run_status.set_thread_status_if_no_active_runs",
                new_callable=AsyncMock,
            ),
...
class TestSafeSerialize:
    def test_returns_serialized_output(self) -> None:
        with patch("agent_server.usecase.execution.run_status._serializer") as mock_ser:

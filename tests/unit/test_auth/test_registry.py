...
from agent_server.auth.registry import (
    EXEMPT_PATHS,
    ROUTE_AUTH_MAP,
    SELF_DISPATCHING,
    lookup_route_auth,
...
def test_self_dispatching_routes_actually_authorize_themselves() -> None:
    ...
    import agent_server.controller.http.routers

    api_dir = pathlib.Path(agent_server.controller.http.routers.__file__).parent
...
def test_registry_matches_spec_snapshot_exactly() -> None:
    ...
    *right* one. Changing a tuple must update this snapshot too, which is the
    ...
    assert ROUTE_AUTH_MAP == _SPEC_TUPLES, (
        ...
        "agent_server/auth/AUTH_DISPATCH_SPEC.md."
    )

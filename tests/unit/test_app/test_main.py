...
from agent_server.infra.observability.base import get_observability_manager
...
async def test_lifespan_registers_otel_provider(monkeypatch):
    ...
    # 1. Reload settings
    import agent_server.config.settings as settings_module

    importlib.reload(settings_module)

    # 2. Reload otel module (creates new Provider class/instance)
    import agent_server.infra.observability.otel as otel_module

    importlib.reload(otel_module)
    # 3. Reload setup module (crucial! so it imports the NEW otel_provider)
    import agent_server.infra.observability.setup as setup_module
    from agent_server.infra.observability.otel import OpenTelemetryProvider
    ...
    with (
        patch("agent_server.app.main.run_migrations_async", new_callable=AsyncMock),
        patch("agent_server.app.main.db_manager") as mock_db_manager,
        patch("agent_server.app.main.get_langgraph_service") as mock_get_langgraph_service,
    ):
...
async def test_lifespan_calls_required_initialization():
    ...
    with (
        ...
        patch("agent_server.app.main.db_manager") as mock_db_manager,
        patch("agent_server.app.main.get_langgraph_service") as mock_get_langgraph_service,
        patch("agent_server.app.main.setup_observability") as mock_setup_observability,
    ):
        # Setup mocks
        mock_db_manager.initialize = AsyncMock()
        mock_db_manager.close = AsyncMock()
...
async def test_lifespan_skips_migrations_when_disabled(monkeypatch):
    ...
    with (
        ...
        patch("agent_server.app.main.db_manager") as mock_db_manager,
        patch("agent_server.app.main.get_langgraph_service") as mock_get_langgraph_service,
        patch("agent_server.app.main.setup_observability"),
    ):
        mock_db_manager.initialize = AsyncMock()
        mock_db_manager.close = AsyncMock()
...
async def test_lifespan_starts_ttl_sweeper_when_configured() -> None:
    ...
    with (
        ...
        patch("agent_server.app.main.get_langgraph_service") as mock_get_langgraph_service,
        patch("agent_server.app.main.setup_observability"),
        patch("agent_server.app.main.get_thread_ttl_config", return_value=MagicMock()),
        patch("agent_server.app.main.thread_ttl_sweeper") as mock_sweeper,
    ):
        mock_db_manager.initialize = AsyncMock()
        mock_db_manager.close = AsyncMock()
        mock_langgraph_service = MagicMock()
...
async def test_lifespan_skips_ttl_sweeper_without_config() -> None:
    ...
    with (
        ...
        patch("agent_server.app.main.get_langgraph_service") as mock_get_langgraph_service,
        patch("agent_server.app.main.setup_observability"),
        patch("agent_server.app.main.get_thread_ttl_config", return_value=None),
        patch("agent_server.app.main.thread_ttl_sweeper") as mock_sweeper,
    ):
        mock_db_manager.initialize = AsyncMock()
        mock_db_manager.close = AsyncMock()

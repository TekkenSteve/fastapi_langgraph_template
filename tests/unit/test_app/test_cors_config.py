...
def isolated_module_reload(tmp_path, monkeypatch):
    ...
    # Save and update the settings GRAPHS_CONFIG to point to our temp config file
    from agent_server.config.settings import settings

    original_agent_server_config = settings.app.GRAPHS_CONFIG
    # Use object.__setattr__ to bypass pydantic's frozen model protection
    object.__setattr__(settings.app, "GRAPHS_CONFIG", str(tmp_path / "langgraph.json"))

    yield tmp_path

    # Restore original GRAPHS_CONFIG
    object.__setattr__(settings.app, "GRAPHS_CONFIG", original_agent_server_config)
    ...
    # agent_server.config.graph_config/main objects that would leak into later tests.
...
def reload_main_module() -> types.ModuleType:
    """Clear agent_server.app.main and agent_server.config.graph_config from cache and reimport main.
    ...
    """
    # Remove main, config, and the agent_server package reference to force fresh imports
    # The order matters - remove submodules first, then package
    modules_to_remove = [
        k
        for k in list(sys.modules.keys())
        if k in ("agent_server.app.main", "agent_server.config.graph_config", "agent_server")
    ]
    ...
    import agent_server.app.main as main_module

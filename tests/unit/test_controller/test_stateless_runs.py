...
from agent_server.controller.http.routers.stateless_runs import (
...
from agent_server.usecase.execution.run_cleanup import (
...
class TestDeleteThreadById:
    ...
    async def test_deletes_thread_with_cascade(
        ...
        with (
            patch("agent_server.usecase.execution.run_cleanup._get_session_maker", return_value=mock_maker),
            patch("agent_server.usecase.execution.run_cleanup.db_manager", mock_db_manager),
        ):
    ...
    async def test_cancels_active_runs_before_delete(self, mock_session: AsyncMock) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_cleanup._get_session_maker", return_value=mock_maker),
            patch(
                "agent_server.usecase.execution.run_cleanup.streaming_service.cancel_run",
                new_callable=AsyncMock,
            ) as mock_cancel,
            patch("agent_server.usecase.execution.run_cleanup.active_runs", {run_id: mock_task}),
        ):
            await _delete_thread_by_id(thread_id, user_id)
    ...
    async def test_noop_when_thread_not_found(self, mock_session: AsyncMock) -> None:
        ...
        with patch("agent_server.usecase.execution.run_cleanup._get_session_maker", return_value=mock_maker):
            await _delete_thread_by_id("nonexistent", "user")
    ...
    async def test_handles_cancelled_error_on_task_await(self, mock_session: AsyncMock) -> None:
        ...
        with (
            ...
            patch("agent_server.usecase.execution.run_cleanup.active_runs", {run_id: fut}),
        ):
    ...
    async def test_logs_infra_error_on_task_await(self, mock_session: AsyncMock) -> None:
        ...
        with (
            ...
            patch(
                "agent_server.usecase.execution.run_cleanup.active_runs",
...
class TestCleanupAfterBackgroundRun:
    ...
    async def test_awaits_task_then_deletes(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_cleanup.active_runs", {run_id: task}),
            patch(
                "agent_server.usecase.execution.run_cleanup.delete_thread_by_id",
                new_callable=AsyncMock,
            ) as mock_delete,
        ):
    ...
    async def test_deletes_thread_when_no_task_in_active_runs(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_cleanup.active_runs", {}),
            patch(
                "agent_server.usecase.execution.run_cleanup.delete_thread_by_id",
                new_callable=AsyncMock,
            ) as mock_delete,
    ...
    async def test_deletes_thread_when_wait_times_out(self) -> None:
        ...
        with (
            patch(
                "agent_server.usecase.execution.run_cleanup.executor.wait_for_completion",
                new_callable=AsyncMock,
...
class TestStatelessWaitForRun:
    ...
    async def test_delegates_and_deletes_thread(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-1"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.wait_for_run",
                new_callable=AsyncMock,
                return_value=mock_response,
            ) as mock_wait,
            patch(
                "agent_server.controller.http.routers.stateless_runs.delete_thread_by_id",
                new_callable=AsyncMock,
            ) as mock_delete,
        ):
    ...
    async def test_keeps_thread_when_requested(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-2"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.wait_for_run",
                new_callable=AsyncMock,
                return_value=mock_response,
    ...
    async def test_cleans_up_on_error(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-3"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.wait_for_run",
                new_callable=AsyncMock,
    ...
    async def test_cleanup_failure_does_not_mask_original_error(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-err"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.wait_for_run",
                new_callable=AsyncMock,
...
class TestStatelessStreamRun:
    ...
    async def test_delegates_and_wraps_body_for_cleanup(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-4"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
            ) as mock_stream,
            patch(
    ...
    async def test_passes_through_when_keep(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-5"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
    ...
    async def test_early_disconnect_keeps_thread(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-disc"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
    ...
    async def test_slow_client_disconnect_with_finished_run_schedules_cleanup(
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-slow"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
            ...
            patch(
                "agent_server.controller.http.routers.stateless_runs.broker_manager.get_broker",
    ...
    async def test_slow_client_disconnect_with_active_run_keeps_thread(
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-active"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
    ...
    async def test_slow_client_disconnect_without_run_id_keeps_thread(
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-noid"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
    ...
    async def test_stream_cleanup_failure_is_logged_not_raised(self, mock_user: User) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-cleanup"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_and_stream_run",
                new_callable=AsyncMock,
                return_value=mock_response,
...
class TestStatelessCreateRun:
    ...
    async def test_delegates_and_schedules_cleanup(self, mock_user: User, mock_session: AsyncMock) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-6"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_run",
                new_callable=AsyncMock,
                return_value=mock_run,
            ) as mock_create,
            patch("agent_server.controller.http.routers.stateless_runs.asyncio.create_task") as mock_create_task,
        ):
    ...
    async def test_no_cleanup_when_keep(self, mock_user: User, mock_session: AsyncMock) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.stateless_runs.uuid4", return_value="eph-thread-7"),
            patch(
                "agent_server.controller.http.routers.stateless_runs.create_run",
                new_callable=AsyncMock,
                return_value=mock_run,

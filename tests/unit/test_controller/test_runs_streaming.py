...
from agent_server.controller.http.routers.runs import create_and_stream_run, stream_run
from agent_server.domain import RunCreate, User
...
class TestRunsStreamingEndpoints:
    ...
    async def test_create_and_stream_run_success(
        ...
        with (
            ...
            patch(
                "agent_server.controller.http.routers.runs.streaming_service.stream_run_execution"
            ) as mock_stream_exec,
            patch(
                "agent_server.controller.http.routers.runs._get_session_maker",
                return_value=_make_session_maker(mock_session),
            ),
        ):
    ...
    async def test_create_and_stream_run_disconnect_handler_wiring(
        ...
        with (
            patch("agent_server.usecase.execution.run_preparation._validate_resume_command", new_callable=AsyncMock),
            patch("agent_server.usecase.execution.run_preparation.get_langgraph_service") as mock_lg_service,
            patch("agent_server.usecase.execution.run_preparation.resolve_assistant_id", return_value="test-assistant"),
            patch("agent_server.usecase.execution.run_preparation.update_thread_metadata", new_callable=AsyncMock),
            patch("agent_server.usecase.execution.run_preparation.set_thread_status", new_callable=AsyncMock),
            patch("agent_server.usecase.execution.run_preparation.uuid4", return_value=run_id),
            patch(
            ...
            patch(
                "agent_server.controller.http.routers.runs.broker_manager.request_cancel", new_callable=AsyncMock
            ) as mock_cancel,
            patch(
                "agent_server.controller.http.routers.runs._get_session_maker",
                return_value=_make_session_maker(mock_session),
    ...
    async def test_create_and_stream_run_handler_swallows_broker_errors(
        ...
        with (
            ...
            patch(
                "agent_server.controller.http.routers.runs.broker_manager.request_cancel",
                new_callable=AsyncMock,
    ...
    async def test_create_and_stream_run_invokes_create_run_auth_handler(
        ...
        with (
            patch("agent_server.controller.http.routers.runs.handle_event", new_callable=AsyncMock) as mock_handle,
            patch("agent_server.controller.http.routers.runs._prepare_run", new_callable=AsyncMock) as mock_prepare,
            patch(
                "agent_server.controller.http.routers.runs.streaming_service.stream_run_execution",
                return_value=_single_event_stream(),
            ),
            patch(
                "agent_server.controller.http.routers.runs._get_session_maker",
                return_value=_make_session_maker(mock_session),
    ...
    async def test_execute_run_async_error_handling(self, mock_user: User, mock_session: AsyncMock) -> None:
        ...
        from agent_server.usecase.execution.run_executor import execute_run as execute_run_async
        from agent_server.usecase.execution.run_job import RunExecution, RunIdentity, RunJob
        from agent_server.usecase.streaming.broker import RunBroker, broker_manager
        ...
        with (
            patch("agent_server.usecase.execution.run_executor.get_langgraph_service") as mock_lg_service,
            patch(
                "agent_server.usecase.execution.run_executor.stream_graph_events",
            ...
            patch(
                "agent_server.usecase.execution.run_executor.finalize_run", new_callable=AsyncMock, return_value=True
            ),
        ):

...
from agent_server.controller.http.routers.crons import _authorize_cron_create, _trigger_first_run
from agent_server.domain import Run, User
from agent_server.domain.crons import CronCreate
...
class TestTriggerFirstRun:
    ...
    async def test_schedules_cleanup_for_stateless_cron(
        ...
        with (
            patch("agent_server.controller.http.routers.crons.uuid4", return_value="eph-thread-1"),
            patch(
                "agent_server.controller.http.routers.crons._prepare_run",
                new_callable=AsyncMock,
                return_value=("run-001", mock_run, Mock()),
            ) as mock_prepare,
            patch("agent_server.controller.http.routers.crons.schedule_background_cleanup") as mock_schedule,
        ):
    ...
    async def test_skips_cleanup_when_keep_requested(
        ...
        with (
            patch("agent_server.controller.http.routers.crons.uuid4", return_value="eph-thread-keep"),
            patch(
                "agent_server.controller.http.routers.crons._prepare_run",
                new_callable=AsyncMock,
                return_value=("run-001", mock_run, Mock()),
    ...
    async def test_deletes_ephemeral_thread_when_initial_run_setup_fails(
        ...
        with (
            patch("agent_server.controller.http.routers.crons.uuid4", return_value="eph-thread-fail"),
            patch(
                "agent_server.controller.http.routers.crons._prepare_run",
                new_callable=AsyncMock,
            ...
            patch(
                "agent_server.controller.http.routers.crons.delete_thread_by_id", new_callable=AsyncMock
            ) as mock_delete,
...
class TestAuthorizeCronCreate:
    ...
    async def test_stateless_create_fires_full_chain(self, user: User, request_body: CronCreate) -> None:
        with patch("agent_server.controller.http.routers.crons.handle_event", new_callable=AsyncMock) as mock_handle:
    ...
    async def test_chain_stops_when_assistants_read_denies(self, user: User, request_body: CronCreate) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.crons.handle_event", side_effect=fake_handle) as mock_handle,

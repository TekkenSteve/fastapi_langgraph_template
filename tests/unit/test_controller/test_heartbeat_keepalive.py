...
from agent_server.controller.http.routers.runs import join_run
from agent_server.domain import User
from agent_server.repo.orm import Run as RunORM
from agent_server.usecase.execution.run_waiters import heartbeat_wait_body, read_run_result
...
class TestReadRunResult:
    ...
    async def test_returns_output_for_success(self) -> None:
        ...
        with patch("agent_server.usecase.execution.run_waiters._get_session_maker", return_value=maker):
    ...
    @pytest.mark.asyncio
    async def test_timeout_status_run_reports_the_timeout(self) -> None:
        session = AsyncMock()
        session.scalar.return_value = _make_run_orm(status="timeout", output={})
        maker = _make_session_maker(session)
...
class TestHeartbeatWaitBody:
    ...
    async def test_immediate_completion_yields_json_only(self) -> None:
        ...
        with (
            patch("agent_server.usecase.execution.run_waiters._get_session_maker", return_value=maker),
...
class TestJoinRunEndpoint:
    ...
    async def test_terminal_run_returns_immediately(self) -> None:
        ...
        with patch("agent_server.controller.http.routers.runs._get_session_maker", return_value=maker):
    ...
    async def test_pending_run_returns_streaming_response(self) -> None:
        ...
        with (
            patch("agent_server.controller.http.routers.runs._get_session_maker", return_value=multi_maker),
            patch("agent_server.usecase.execution.run_waiters._get_session_maker", return_value=multi_maker),
            patch("agent_server.usecase.execution.run_waiters.executor") as mock_executor,
            patch("agent_server.usecase.execution.run_waiters.settings") as mock_settings,
        ):
            mock_executor.wait_for_completion = AsyncMock()

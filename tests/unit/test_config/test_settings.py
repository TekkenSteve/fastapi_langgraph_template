...
from agent_server.config.settings import (
    AppSettings,
    CronSettings,
    DatabaseSettings,
    RedisSettings,
...
class TestDatabaseURLSupport:
    ...
    def test_defaults_when_no_database_url(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert db.POSTGRES_DB == "agent_server"
        assert "postgres:postgres@localhost:5432/agent_server" in db.database_url

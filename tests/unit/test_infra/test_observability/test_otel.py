...
class TestOpenTelemetryProviderInit:
    ...
    def test_init_disabled_by_default(self):
        ...
        with patch("agent_server.infra.observability.otel.settings") as mock_settings:
    ...
    def test_init_ignores_unknown_targets(self):
        ...
        with patch("agent_server.infra.observability.otel.settings") as mock_settings:
            ...
            with patch("agent_server.infra.observability.otel.logger") as mock_logger:
                provider = OpenTelemetryProvider()
...
class TestOpenTelemetryProviderSetup:
    ...
    def mock_deps(self):
        ...
        with (
            patch("agent_server.infra.observability.otel.TracerProvider") as mock_tp,
            patch("agent_server.infra.observability.otel.BatchSpanProcessor") as mock_bsp,
            patch("agent_server.infra.observability.otel.ConsoleSpanExporter") as mock_cse,
            patch("agent_server.infra.observability.otel.LangChainInstrumentor") as mock_lci,
            patch("agent_server.infra.observability.otel.trace") as mock_trace,
            patch("agent_server.infra.observability.otel.Resource") as mock_resource,
            patch("agent_server.infra.observability.otel.settings") as mock_settings,
        ):
            # Setup defaults

"""Tests for agent_server.repo.migrations module."""
...
from agent_server.repo import migrations as migrations_mod
from agent_server.repo.migrations import find_alembic_ini, get_alembic_config
...
class TestFindAlembicIni:
    ...
    def test_finds_alembic_ini_in_package(self, tmp_path, monkeypatch):
        """Should find alembic.ini bundled with the agent_server package."""
        ...
        # Should be in the package directory (agent_server/)
...
class TestRunMigrations:
    ...
    def test_run_migrations_calls_alembic_upgrade(self, tmp_path, monkeypatch):
        ...
        with patch("agent_server.repo.migrations.command") as mock_command:
            from agent_server.repo.migrations import run_migrations
    ...
    async def test_run_migrations_async_dispatches_to_lockfree_path(self):
        ...
        with patch("agent_server.repo.migrations.asyncio.to_thread") as mock_to_thread:
            from agent_server.repo.migrations import (
...
class TestRunMigrationsIfNeeded:
    ...
    def test_skips_upgrade_when_database_at_head(self, tmp_path, monkeypatch):
        ...
        with (
            patch("agent_server.repo.migrations._is_database_up_to_date", return_value=True) as mock_check,
            patch("agent_server.repo.migrations.command") as mock_command,
        ):
            from agent_server.repo.migrations import run_migrations_if_needed
    ...
    def test_runs_upgrade_when_database_behind_head(self, tmp_path, monkeypatch):
        ...
        with (
            patch("agent_server.repo.migrations._is_database_up_to_date", return_value=False),
            patch("agent_server.repo.migrations.command") as mock_command,
        ):
            from agent_server.repo.migrations import run_migrations_if_needed
    ...
    def test_falls_back_to_upgrade_when_precheck_fails(self, tmp_path, monkeypatch):
        ...
        with (
            patch(
                "agent_server.repo.migrations._is_database_up_to_date",
...
class TestIsDatabaseUpToDate:
    ...
    def test_returns_true_when_revisions_match(self):
        from agent_server.repo.migrations import _is_database_up_to_date
        ...
        with (
            patch("agent_server.repo.migrations.psycopg.connect", return_value=connect_cm),
            patch("agent_server.repo.migrations.ScriptDirectory.from_config", return_value=script),
        ):
    ...
    def test_returns_true_when_script_directory_empty(self):
        ...
        with (
            patch("agent_server.repo.migrations.ScriptDirectory.from_config", return_value=script),
            patch("agent_server.repo.migrations.psycopg.connect") as mock_connect,
        ):
            assert _is_database_up_to_date(cfg) is True
    ...
    def test_uses_libpq_url_for_multihost_compatibility(self):
        ...
        with (
            patch("agent_server.repo.migrations.psycopg.connect", return_value=connect_cm) as mock_connect,
            patch("agent_server.repo.migrations.ScriptDirectory.from_config", return_value=script),
        ):
